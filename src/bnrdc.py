...
val_txt = "./checkpoints/val.txt"
if os.path.exists(val_txt):
    print("文件存在")
else:
    file = open(val_txt, "w", encoding='utf-8')
    print("创建成功")
...
class BNRDC():
    def __init__(self, config):
        ...
        self.results_path = os.path.join(config.PATH, 'celeba')
    ...
    def train(self):
        ...
        while(keep_training):
            ...
            for items in train_loader:
                ...
                # transforms.ToPILImage()(outputs_merged[0]).show()
                # backward
    ...
    def eval(self):
        ...
        with torch.no_grad():
            ...
            with open(val_txt, "a+") as f:
                f.write('iteration:{} ave_psnr:{}  ave_ssim:{} ave_l1:{}  ave_lpips:{}'.format(
                                            iteration,
                                            np.average(psnr_all),
                                            np.average(ssim_all),
                                            np.average(l1_list),
    ...
    def test(self):
        ...
        with torch.no_grad():
            for items in test_loader:
                ...
                # 打印单张图片的推理时间
                print(f"Inference time for image {index}: {inference_time:.4f} seconds")

                # 如果已经处理了1000张图片，计算平均推理时间并退出循环
                if index == 10:
                    average_inference_time = total_inference_time / 10
                    print(f"Average inference time for 1000 images: {average_inference_time:.4f} seconds")
                    break
    ...
    def metric(self, gt, pre):
        ...
        ssim = compare_ssim(gt, pre, multichannel=True)
