    ...
    def load(self, type):
        ...
        if os.path.exists(self.gen_weights_path):
            ...
            if torch.cuda.is_available():
                data = torch.load(self.gen_weights_path, map_location="cuda:0")
            else:
        ...
        if self.config.MODE == 1 and os.path.exists(self.dis_weights_path):
            ...
            if torch.cuda.is_available():
                data = torch.load(self.dis_weights_path, map_location="cuda:0")
            else:
...
class InpaintingModel(BaseModel):
    ...
    def process(self, images, masks):
      ...
      #  transforms.ToPILImage()(outputs[0]).show()
        gen_loss = 0
        dis_loss = 0
